# Helper functions for plotting ASR results, 
# adapted from the Topiary library
# See https://topiary-asr.readthedocs.io/en/latest/topiary.draw.html
...
def plot_ancestor_data(df_anc,
    ...
    Slightly adapted from topiary.draw.ancestor_data.plot_ancestor_data
    Parameters
    ----------
    ...
    ax[0].plot(df_nogap.site,df_nogap.ml_pp,".",color=GRAY,markersize=8, alpha=0.9)
    ...
    contiguous = np.split(sites,np.where(np.diff(sites) != 1)[0]+1)
    for c in contiguous:
        ax[0].plot(df_anc.site.iloc[c],df_anc.ml_pp.iloc[c],color=GRAY,lw=2)
...
def plot_conservation_data(df_anc, 
                       width_ratio=5,
                       anc_name=None,
                       anc_data_string=None,
                       close_plot=False):
    """
    Create a summary plot of per-site conservation. 
    Adapted from topiary.draw.ancestor_data.plot_ancestor_data
    Parameters
    ----------
    df_anc : pandas.DataFrame
        ancestral data frame
    ...
    # Plot Equality #ML and alt pp points
    ax[0].plot(df_nogap.site,df_nogap.equal,".",color=BLUE,markersize=8, alpha=0.9)
    #ax[0].plot(df_nogap.site,df_nogap.alt_pp,".",color="red",markersize=8)
    ...
    contiguous = np.split(sites,np.where(np.diff(sites) != 1)[0]+1)
    for c in contiguous:
        ax[0].plot(df_anc.site.iloc[c],df_anc.equal.iloc[c],color=BLUE,lw=2, alpha=0.9)

    # Draw histograms for ml and alt pp in right plot
    max_ml_counts = _draw_histogram(df_nogap.equal,ax[1],color=BLUE)
    print(df_nogap.equal.mean())


    # Clean up axes for main plot
    ax[0].set_xlabel("alignment site")
    ax[0].set_ylabel("Conservation")
    #ax[0].set_title("Comparison w/ mouse Elfn1")
    ax[0].set_xlim(np.min(df_anc.site),np.max(df_anc.site))
    ax[0].spines["right"].set_visible(False)
    ax[0].spines["top"].set_visible(False)
    #ax[0].set_ylim(-0.05,1.1)
    ...
    # Clean up axes for histogram plot
    ax[1].set_xlim(0,max_ml_counts*1.1)
    ax[1].spines["right"].set_visible(False)
    ax[1].spines["top"].set_visible(False)
    ax[1].spines["bottom"].set_visible(True)
    ax[1].spines["left"].set_visible(False)
